...
if ( is_uploaded_file($tempfile) ) { //ファイルがアップロードされている（is_uploaded_fileがtrue）
    if( preg_match('/\d{2}_st\d{2}_ver[1-9]\.(?i)pdf/', $_FILES['fname']['name']) ){ //正規表現とマッチする（preg_matchがtrue，\d{2}は二桁の半角数字，[1-9]は1～9の半角数字1桁，(?i)は以後大文字小文字を区別しない）
        ...
        echo "<p>レポートの表紙とチェックシートとレポート本体を1つのPDFファイルとして保存し，ファイル名を「XX_stYY_verZ.pdf」としてアップロードすること．<br>このとき，XXには2桁の半角数字の班番号（01, 02, ..., 09, 10），YYには2桁の半角数字の出席番号（01, 02, ..., 44, 45），Zにはバージョン番号（初回提出は1，再レポート提出は2，再々レポート提出は3，...）がそれぞれ入る．<br>例：1班出席番号12番の初回レポート提出時は「01_st12_ver1.pdf」，1班出席番号12番の再レポート提出時は「01_st12_ver2.pdf」<br>「全角英数文字を使う」，「アンダースコア（_）とハイフン（-）を間違える」等の誤ったファイル名ではアップロードエラーが表示される．<br>また，班番号や出席番号を間違えた場合は減点するので注意せよ．<br>同一のファイル名で再アップロードするとファイルは上書きされる．</p>";
    }
}
...
echo "現在サーバにアップロードされているファイル一覧：<br>";
if (is_dir($dir)) {
    if ($dh = opendir($dir)) {
